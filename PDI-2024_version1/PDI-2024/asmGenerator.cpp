...
namespace ASMGenerator {
	void asmGenerator(LexA::LEX lex, wchar_t outfile[]) {
		...
		stack<pair<string, int>> parms; // стек дл€ парметров функции и типов
		map<string, vector<string>> funcwithSTR; // функции со строковыми параметрами
		stack<string> STRp; // дл€ названий переменных строкового типа в функции
		stack<string> STRs; // дл€ названий переменных стркового типа, которые €вл€ютс€ ресурсами
		stack<string> OSTPARMS; // остальные параметры в функции
		stack<string> stk; // стек дл€ отслеживани€ текущего блока(функци€, условный, цикл)
		stack<int> ifi; // стек дл€ отслеживани€ текущего номера услови€
		stack<int> whilei;// стек дл€ отслеживани€ текущего номера цикла
		int num_of_ret = 0,
			num_of_logical = 0,
			num_of_if = 0,
			num_of_while = 0,
